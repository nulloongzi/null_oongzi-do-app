...
> | versionCode | 상태 |
> |---|---|
> | **16 (2.7.0)** | **빌드됨, 업로드 전** (2026-09-20). Release AAB **run #15** 산출물 — main(커밋 `2587762`, PR #46 머지분), `build_number=16` 입력으로 빌드. 릴스 커버 카드 + 릴스 계측 + 안치기 6·9인제 개편. 업로드·게시하면 이 줄을 갱신할 것. (run #14 는 머지 전 브랜치에서 같은 내용을 먼저 뽑아 본 것 — 올리지 말 것.) |
...
> 안치기 문단은 **그 개편이 16 에 실릴 때만** 쓴다. 다음 빌드로 밀리면 이 문단을
...
```
🔒 대략적인 위치만 공개할 수 있어요
학교나 공공 체육관을 빌려 쓰는 팀을 위해, 지도에 정확한 핀 대신 동네 범위로 표시하는 선택지를 넣었습니다. 주소도 시·군·구까지만 보여요. 팀 등록·수정 화면에서 켜면 됩니다.

🙋 팀 관리자를 최대 3명까지
한 팀을 여러 명이 함께 관리할 수 있어요.
• 팀 인증을 신청하신 분은 자동으로 관리자가 됩니다
• 이미 인증된 팀은 팀 상세에서 '관리자 신청' — 증빙 사진을 올리면 확인 후 권한을 드려요
• 더는 팀을 맡지 않게 되면 스스로 빠질 수 있습니다
```

</details>

<details><summary>2.5.2(13) 출시 노트 — 13 을 건너뛴 사용자용</summary>

```
쓰다가 불편했던 것들을 고쳤어요.

• 팀 정보를 수정할 때 '모집 대상'의 기타 메모와 운동 시간이 비어 있던 문제를
  고쳤어요. 그대로 저장하면 적어두신 메모가 지워질 수 있었습니다
• 도시락통에서 팀을 고르면 지도도 그 팀 위치로 함께 이동합니다
• '월, 수, 금 19:00~22:00' 처럼 요일을 나열해 적은 일정이 마지막 요일에만
  표시되던 문제 수정
• 주소 대신 '석관중', '잠실학생체육관' 같은 체육관 이름으로도 위치를 찾을 수
  있어요. 찾으면 주소칸도 실제 도로명으로 채워집니다
```

</details>

<details><summary>2.5.1(12) 출시 노트 — 2.3.0 에서 건너뛴 사용자용</summary>
...
**13(2.5.2)에 새로 들어가는 것 — 2.5.1(12) 이후**

| PR | 내용 | 사용자에게 보이나 |
|---|---|---|
| [#39](https://github.com/nulloongzi/null_oongzi-do-app/pull/39) | 장소 이름으로 찾았으면 주소칸을 실제 도로명으로 | ✅ |
| [#40](https://github.com/nulloongzi/null_oongzi-do-app/pull/40) | 수정 폼 프리필 복원 · 도시락통에서 지도 이동 · 요일 나열 파싱 | ✅ |
| #37 · #38 | 스토어 등록정보 문서 정리 | ❌ 문서만 |

<details><summary>12(2.5.1) 에 나갔던 미출시분 (2.3.0 이후) — 이력</summary>
...
> 브랜치 제한은 없다 — `Run workflow` 에서 ref 를 고를 수 있어 머지 전 브랜치에서도
> 뽑힌다(실제로 16 은 그렇게 뽑았다). 다만 스토어에 낼 물건은 머지 후 main 에서
> 뽑는 쪽이 낫다: 이 문서의 번호 이력이 "run #N(커밋)" 으로 추적되기 때문이다.
> 같은 커밋이면 같은 versionCode 가 나오므로, 이미 올린 버전이면 `build_number`
> 입력으로 올려서 빌드한다.

업로드는 Play Console → 프로덕션 → 새 버전 만들기 → 드래그 업로드.
출시명이 `16 (2.7.0)` 로 잡히는지 확인한다 — 위 번호 이력대로 14 까지는 이미 썼고 15 는 건너뛰었다.
...
## 5. App Links (딥 링크) — 2026-09-15 해결

Play Console 이 "웹 도메인이 앱과 연결되어 있지 않아 딥 링크 1개가 작동하지 않을 수 있음"
을 띄웠던 건이다. **현재 상태: ✅ 도메인 소유권 확인됨 · 모든 링크 작동 중**
(콘솔 딥 링크 화면, 분석 기준 `14 (2.6.0)`).

원인이 두 겹이었고 둘 다 웹 레포(`null_oongzi-do`)에서 고쳤다.

1. **파일이 배포되지 않았다.** `.well-known/assetlinks.json` 은 2026-08-05 부터
   커밋돼 있었지만 URL 은 404 였다. GitHub Pages 는 기본으로 Jekyll 을 거치고,
   Jekyll 은 **점(`.`)으로 시작하는 디렉터리를 빌드 산출물에서 통째로 제외한다.**
   → 레포 루트에 빈 `.nojekyll` 을 두면 파일이 그대로 올라간다.
   커밋해두고 "배포됐다"고 믿기 쉬운 자리다.

2. **지문이 틀렸다.** 파일이 읽히게 된 뒤에도 검증은 계속 실패했다. 등록된 값이
   Play 앱 서명 키가 아니었기 때문이다.

지금 등록돼 있어야 하는 값 (`sha256_cert_fingerprints`):

| 키 | SHA-256 | 왜 필요한가 |
|---|---|---|
| Play 앱 서명 키 | `39:8F:07:A9:BB:46:…:47:7D` | 사용자가 스토어에서 받는 APK 가 이 키로 서명된다. **이게 없으면 검증이 통과하지 않는다.** |
| 업로드 키 | `52:57:06:75:CE:4E:…:6E:0A` | CI 가 AAB 에 찍는 키(`CN=sungwon yoo`). 로컬 빌드·내부 테스트로 깐 앱에서도 딥 링크가 잡히게 한다. |

앱 서명 키 값은 **Play Console → 테스트 및 출시 → 설정 → 앱 무결성 → 앱 서명** 에서
읽는다. '앱 서명 키 인증서' 의 지문 버튼은 클립보드 복사 전용이라 화면에 값이 안 뜨는데,
같은 페이지 하단의 **'디지털 애셋 링크 JSON' 스니펫**에 값이 그대로 들어 있다.

함께 지운 것: `6D:03:5A:A1:…:45:B0`. 2026-08-05 도메인 전환(웹 #46)에 설명 없이
들어왔고 Play Console 어디에도 없는 값이었다. 지문 하나는 "이 키로 서명된
`com.nulloongzi.nulloongzido` 는 이 도메인의 링크를 가로챌 수 있다" 는 허가라,
정체를 모르는 채로 남겨둘 이유가 없다.

### 확인 방법

```
curl -s "https://digitalassetlinks.googleapis.com/v1/statements:list?source.web.site=https://do.nulloongzi.com&relation=delegate_permission/common.handle_all_urls"
```

- `ERROR_CODE_FETCH_ERROR` / 404 → 1번 문제(파일이 안 올라감)
- statement 는 나오는데 지문이 다름 → 2번 문제
- **주의**: 이 API 는 결과를 10분~1시간 캐시한다(`maxAge`). 배포 직후 옛 값이 나오거나
  값이 오락가락하는 건 정상이다. **원본(main)이 맞는지부터 보고**, 캐시가 풀리길 기다린다.

키를 교체하면(앱 서명 키 업그레이드, 업로드 키 재발급) 이 파일도 같이 고쳐야 한다.
