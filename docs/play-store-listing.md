...
> | versionCode | 상태 |
> |---|---|
> | 15 (2.7.0) | **빌드 예정** (pubspec 2026-09-17). 릴스 커버 카드(WebView 제거) + 릴스 계측. 게시되면 이 줄을 갱신할 것. |
...
>
> 번호는 재사용할 수 없다. 14 가 게시됐으므로 다음은 15 다(pubspec 에 반영됨).
>
> 아래가 15(2.7.0) 의 노트 — **2.6.0 이후 변경분만**. 14 의 노트는 접어 뒀다.
...
출시명이 `15 (2.7.0)` 로 잡히는지 확인한다 — 위 번호 이력대로 14 까지는 이미 썼다.
