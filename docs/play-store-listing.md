...
> | versionCode | 상태 |
> |---|---|
> | **14 (2.6.0)** | **업로드됨** (2026-09-15 확인). Release AAB run #12(커밋 `ec0bb38`) 산출물. 팀 관리자 권한 + 위치 공개 수준(아래 출시 노트). 콘솔 딥 링크 화면이 분석 기준을 `14 (2.6.0)` 으로 표시하므로 Play 가 번들을 처리한 것은 확실하다 — **프로덕션 게시 여부는 따로 확인할 것.** |
> | 13 (2.5.2) | 업로드함. **게시 여부는 콘솔에서 확인할 것** — 이 문서는 콘솔에서 확인한 값만 적는데, 13 의 게시 상태는 아직 확인하지 못했다. |
...
>
> 번호는 재사용할 수 없다. 14 를 게시하고 나면 다음은 15 다.
>
> 이번 출시 노트는 **2.5.2 이후 변경분만** 담는다. 다만 13 의 게시 여부가
> 미확인이라, 콘솔에서 13 이 게시된 적 없는 것으로 확인되면 아래 접힌
> 2.5.2 노트를 이 본문 뒤에 이어 붙여야 한다 — 그 사용자들은 13 을 건너뛴다.
...
출시명이 `14 (2.6.0)` 로 잡히는지 확인한다 — 위 번호 이력대로 13 까지는 이미 썼다.
